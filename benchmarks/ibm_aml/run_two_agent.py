"""Run the two-agent detector over retrieved IBM AML contexts."""
...
from benchmarks.ibm_aml.deepseek_agent import DEFAULT_KEY_PATH, DEFAULT_MODEL
...
from benchmarks.ibm_aml.two_agent import classify_query_with_two_agents, summarize_two_agent_decisions


def run_two_agent_experiment(
    csv_path: str | Path,
    *,
    output_dir: str | Path = "outputs/ibm_aml/two_agent",
    ...
    """Run the two-agent detector on a limited set of positive queries."""
    ...
    summary = summarize_two_agent_decisions(decisions)
    summary.update(
        {
            "provider": provider,
            "analyzer_model": analyzer_model,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="outputs/ibm_aml/two_agent")
    parser.add_argument(
        "--retrieval-mode",
        default="timeindex",
    ...
    parser.add_argument("--analyzer-model", default=DEFAULT_MODEL)
...
def main() -> None:
    args = parse_args()
    summary = run_two_agent_experiment(
        args.csv_path,
